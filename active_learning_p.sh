...
if [ "$custom_relax" = true ]; then
 ...
 cp $cmpd_pth/KPOINTS_st .vaspy/KPOINTS
else
 cp $pth/utils/INCAR_st ./vaspy/INCAR
 cp $pth/utils/KPOINTS ./vaspy/KPOINTS
fi
...
#C Grade and select
 n_preselected=$(grep "BEGIN_CFG" preselected.cfg_0 | wc -l)
 ...
 if [ $n_preselected -gt 0 ]; then 
  mlp select-add curr.mtp als-train.cfg preselected.cfg_0 diff.cfg --als-filename=state.als
  D=$(grep "BEGIN" diff.cfg | wc -l)
  echo "$D configurations selected for ab initio relaxation."
  cp diff.cfg ./vaspy/.
  rm preselected.cfg_0
  rm selected.cfg
...
#F Re-train MTP on new training set
  echo "Updating MTP."
  srun mlp train curr.mtp als-train.cfg --trained-pot-name=curr.mtp --update-mindist ${training_settings} > training.out
...
#A Active set re-construction and loop
  ...
  3)
   ...
   if [ -z ${rmse+x} ]; then
    ...
    br=$(python $pth/als-conv.py $ALS_conv $rmse $new_rmse)
    rmse=$new_rmse
   fi
   rm delme;;
  4)
