...
if [ $CHK -lt 2 ]; then
 ...
 cp $pth/par/active_learning_p.sh ./.
 cp $pth/par/mlip.ini ./.
 #See comments in active_learning.sh
 if [ -f curr.mtp ]; then
  bash active_learning_p.sh curr.mtp train.cfg ${cfg} ${els[@]}
 else
  bash active_learning_p.sh pot_${LevMTP}_${cmpd}.mtp train.cfg ${cfg} ${els[@]}
 fi
 CHK=2
...
if [ $CHK -lt 3 ]; then
 ...
 echo "Time Start:"
 date
 cp $pth/par/accurate_potentials_p.sh ./.
 #See comments in accurate_potentials.sh
 bash accurate_potentials_p.sh ${cfg} $cmpd $chull_var $LevMTP
 CHK=3
...
if [ $CHK -lt 4 ]; then
 ...
 touch als-train.cfg
 bash active_learning_p.sh pot_${LevMTP}_${cmpd}.mtp train.cfg lowE_robust_relaxed.cfg ${els[@]}
 CHK=4
 ...
 rm delme
 mpirun mlp relax mlip.ini --cfg-filename=lowE_robust_relaxed.cfg --save-relaxed=rx.cfg --min-dist=$mindist $relax_settings
 for i in rx.cfg_*
 do
  cat $i >> ALS_AR_RR.cfg
 done
...
if [ ! -z ${REF_CFG+x} ]; then
 ...
 cd $cmpd_pth/chulls
 bash $pth/par/dft_chulls_p.sh refs
 rm refs_DONE
 cd $cmpd_pth
 if [ ! -f refs_RR.cfg ]; then
  ...
  mpirun mlp relax mlip.ini --cfg-filename=$REF_CFG --save-relaxed=rx.cfg --min-dist=$mindist $relax_settings
  for i in rx.cfg_*
  do
   cat $i >> refs_RR.cfg
  done
 ...
 if [ ! -f refs_AR.cfg ]; then
  ...
  mpirun mlp relax mlip.ini --cfg-filename=$REF_CFG --save-relaxed=rx.cfg --min-dist=$mindist $relaxation_settings
  for i in rx.cfg_*
  do
   cat $i >> refs_AR.cfg
  done
...
#done
if [ $Nels -eq 3 ]; then
 echo "Generating ternary convex hulls prior to DFT relaxation."
