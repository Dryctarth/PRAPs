...
mpirun mlp relax mlip.ini --cfg-filename=relax.cfg --save-relaxed=rx.cfg --min-dist=$mindist $relax_settings
...
if [ "$basic_acc" = true ]; then
 ...
 srun mlp train pot_${4}_${2}.mtp lowE_robust_relaxed.cfg --trained-pot-name=pot_basic_acc.mtp ${training_settings}
